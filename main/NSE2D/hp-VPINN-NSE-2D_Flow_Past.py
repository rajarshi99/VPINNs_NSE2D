...
class VPINN:
    def __init__(self, coord_bound_train, u_bound_train, v_bound_train, coord_train_force, f_1_train, f_2_train, coord_quad_train, quad_weight_train, F_1_exact_total, F_2_exact_total,\
        ...
        for cell in cells:
            ...
            Ntest_elementx = num_total_testfunc[0][0]  # number of test functions in x direction. Data type: int, Size: (1)
            Ntest_elementy = num_total_testfunc[1][0]  # number of test functions in y direction. Data type: int, Size: (1)

            x_quad_element = tf.constant(x1 + (x2-x1)/2*(self.x_quad+1)) # quadrature points in x direction. Data type: tf.constant, Size: (N_quad,1)
            y_quad_element = tf.constant(y1 + (y2-y1)/2*(self.y_quad+1)) # quadrature points in y direction. Data type: tf.constant, Size: (N_quad,1)

            jacobian_x     = ((x2-x1)/2)                                        # jacobian in x direction. Data type: float, Size: (1)
            jacobian_y     = ((y2-y1)/2)                                        # jacobian in y direction. Data type: float, Size: (1)
            jacobian       = ((x2-x1)/2)*((y2-y1)/2)            # jacobian. Data type: float, Size: (1)
            ...
            Res_NN_element_1 = tf.reshape(U_NN_element_diff + U_NN_element_adv - P_NN_element_1, [1,-1]) # residual. Data type: tf.tensor, Size: (1,Ntest_elementx*Ntest_elementy) 
            Res_NN_element_2 = tf.reshape(V_NN_element_diff + V_NN_element_adv - P_NN_element_2, [1,-1]) # residual. Data type: tf.tensor, Size: (1,Ntest_elementx*Ntest_elementy)
...
if __name__ == "__main__":     
    ...
    x_top = 12*lhs(1,N_bound) # Datatype: numpy.ndarray, Size: (N_bound,1), Sample numbers between 0 to 1
    y_top = np.empty(len(x_top))[:,None]            # Datatype: numpy.ndarray, Size: (N_bound,1)
    
    y_top.fill(3)                                   # Fill the array with 1, to denote top boundary
    ...
    x_bottom = 12*lhs(1,N_bound)                       # Datatype: numpy.ndarray, Size: (N_bound,1). Sample numbers between 0 to 1
    ...
    y_right = 3*lhs(1,N_bound)                        # Datatype: numpy.ndarray, Size: (N_bound,1). Sample numbers between 0 to 1
    x_right = np.empty(len(y_right))[:,None]        # Datatype: numpy.ndarray, Size: (N_bound,1)
    
    x_right.fill(12)                                 # Fill the array with 1, to denote right boundary
    ...
    y_left = 3*lhs(1,N_bound)                            # Datatype: numpy.ndarray, Size: (N_bound,1). Sample numbers between 0 to 1
    ...
    x_top = lhs(1,N_bound) + 1 # Datatype: numpy.ndarray, Size: (N_bound,1), Sample numbers between 0 to 1
    y_top = np.empty(len(x_top))[:,None]            # Datatype: numpy.ndarray, Size: (N_bound,1)
    
    y_top.fill(2)                                   # Fill the array with 1, to denote top boundary
    coord_train_top = np.hstack((x_top, y_top))     # Datatype: numpy.ndarray, Size: (N_bound,2), Place x and y coordinates together side by side

    u_bound_top = np.empty(len(x_top))[:,None]        # Datatype: numpy.ndarray, Size: (N_bound,1)
    u_bound_top = obstruction(x_top, y_top)                 # Evaluate the exact solution at boundary points using given dirichlet boundary condition
    u_top_train = u_bound_top                         # Datatype: numpy.ndarray, Size: (N_bound,1), Place the exact solution at boundary points

    v_bound_top = np.empty(len(x_top))[:,None]        # Datatype: numpy.ndarray, Size: (N_bound,1)
    v_bound_top = obstruction(x_top, y_top)                 # Evaluate the exact solution at boundary points using given dirichlet boundary condition
    v_top_train = v_bound_top                         # Datatype: numpy.ndarray, Size: (N_bound,1), Place the exact solution at boundary points

    x_bottom = lhs(1,N_bound) + 1                       # Datatype: numpy.ndarray, Size: (N_bound,1). Sample numbers between 0 to 1
    y_bottom = np.empty(len(x_bottom))[:,None]      # Datatype: numpy.ndarray, Size: (N_bound,1)
    
    y_bottom.fill(1)                                # Fill the array with 0, to denote bottom boundary
    coord_train_bottom = np.hstack((x_bottom, y_bottom)) # Datatype: numpy.ndarray, Size: (N_bound,2), Place x and y coordinates together side by side

    u_bound_bottom = np.empty(len(x_bottom))[:,None]  # Datatype: numpy.ndarray, Size: (N_bound,1)
    u_bound_bottom = obstruction(x_bottom, y_bottom)        # Evaluate the exact solution at boundary points using given dirichlet boundary condition
    u_bottom_train = u_bound_bottom                        # Datatype: numpy.ndarray, Size: (N_bound,1), Place the exact solution at boundary points

    v_bound_bottom = np.empty(len(x_bottom))[:,None]  # Datatype: numpy.ndarray, Size: (N_bound,1)
    v_bound_bottom = obstruction(x_bottom, y_bottom)        # Evaluate the exact solution at boundary points using given dirichlet boundary condition
    v_bottom_train = v_bound_bottom                        # Datatype: numpy.ndarray, Size: (N_bound,1), Place the exact solution at boundary points

    y_right = lhs(1,N_bound) + 1                        # Datatype: numpy.ndarray, Size: (N_bound,1). Sample numbers between 0 to 1
    x_right = np.empty(len(y_right))[:,None]        # Datatype: numpy.ndarray, Size: (N_bound,1)
    
    x_right.fill(2)                                 # Fill the array with 1, to denote right boundary
    coord_train_right = np.hstack((x_right, y_right))  # Datatype: numpy.ndarray, Size: (N_bound,2), Place x and y coordinates together side by side

    u_bound_right = np.empty(len(y_right))[:,None]    # Datatype: numpy.ndarray, Size: (N_bound,1)
    u_bound_right = obstruction(x_right, y_right)           # Evaluate the exact solution at boundary points using given dirichlet boundary condition
    u_right_train = u_bound_right                        # Datatype: numpy.ndarray, Size: (N_bound,1), Place the exact solution at boundary points

    v_bound_right = np.empty(len(y_right))[:,None]    # Datatype: numpy.ndarray, Size: (N_bound,1)
    v_bound_right = obstruction(x_right, y_right)           # Evaluate the exact solution at boundary points using given dirichlet boundary condition
    v_right_train = v_bound_right                        # Datatype: numpy.ndarray, Size: (N_bound,1), Place the exact solution at boundary points

    y_left = lhs(1,N_bound) + 1                            # Datatype: numpy.ndarray, Size: (N_bound,1). Sample numbers between 0 to 1
    x_left = np.empty(len(y_left))[:,None]             # Datatype: numpy.ndarray, Size: (N_bound,1)
    
    x_left.fill(1)                                     # Fill the array with 0, to denote left boundary
    coord_left_train = np.hstack((x_left, y_left))     # Datatype: numpy.ndarray, Size: (N_bound,2), Place x and y coordinates together side by side


    u_bound_left = np.empty(len(y_left))[:,None]         # Datatype: numpy.ndarray, Size: (N_bound,1)
    u_bound_left = obstruction(x_left, y_left)                 # Evaluate the exact solution at boundary points using given dirichlet boundary condition
    u_left_train = u_bound_left                          # Datatype: numpy.ndarray, Size: (N_bound,1), Place the exact solution at boundary points

    v_bound_left = np.empty(len(y_left))[:,None]         # Datatype: numpy.ndarray, Size: (N_bound,1)
    v_bound_left = obstruction(x_left, y_left)                 # Evaluate the exact solution at boundary points using given dirichlet boundary condition
    v_left_train = v_bound_left                          # Datatype: numpy.ndarray, Size: (N_bound,1), Place the exact solution at boundary points

    coord_all_train = np.concatenate((coord_all_train, coord_train_top, coord_train_bottom, coord_train_right, coord_left_train)) # Datatype: numpy.ndarray, Size: (4*N_bound,2), Place all the coordinates together on top of each other

    u_all_train = np.concatenate((u_all_train, u_top_train, u_bottom_train, u_right_train, u_left_train))                    # Datatype: numpy.ndarray, Size: (4*N_bound,1), Place all the exact solution together on top of each other
    v_all_train = np.concatenate((v_all_train, v_top_train, v_bottom_train, v_right_train, v_left_train))                    # Datatype: numpy.ndarray, Size: (4*N_bound,1), Place all the exact solution together on top of each other
    ...
    [x_left, x_right] = [0, 12] # Domain boundaries in x direction. Datatype: float
    [y_bottom, y_up] = [0, 3]
    ...
    grid_x = np.asarray(list(range(3)) + list(range(3,13,3)))
...
#%%
    ...
    if input_data['mlflow_parameters']['use_mlflow']:
        ...
        # move the yaml file 
        with open (f"{output_folder}/input.yaml", "w") as fyaml:
            yaml.dump(input_data, fyaml)
        ...
        mlflow.log_artifact(f"{output_folder}/input.yaml")
