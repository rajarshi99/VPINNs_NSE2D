...
class VPINN:
    def __init__(
        ...
        self.u_tf = tf.compat.v1.placeholder(
            tf.float64, shape=[None, self.utrain.shape[1]]
        )
        self.x_f_tf = tf.compat.v1.placeholder(
            tf.float64, shape=[None, self.xf.shape[1]]
        )
        self.y_f_tf = tf.compat.v1.placeholder(
            tf.float64, shape=[None, self.yf.shape[1]]
        )
        self.f_tf = tf.compat.v1.placeholder(
            tf.float64, shape=[None, self.ftrain.shape[1]]
        )
        self.x_test = tf.compat.v1.placeholder(
            tf.float64, shape=[None, self.xtest.shape[1]]
        )
        self.y_test = tf.compat.v1.placeholder(
            tf.float64, shape=[None, self.ytest.shape[1]]
        )
        self.x_quad = tf.compat.v1.placeholder(
            tf.float64, shape=[None, self.xquad.shape[1]]
        )
        self.y_quad = tf.compat.v1.placeholder(
            tf.float64, shape=[None, self.yquad.shape[1]]
        )
        ...
        for ex in range(self.Nelementx):
            for ey in range(self.Nelementy):
                ...
                x_quad_element = tf.constant(
                    gridx[ex] + (gridx[ex + 1] - gridx[ex]) / 2 * (self.xquad + 1)
                )
                y_quad_element = tf.constant(
                    gridy[ey] + (gridy[ey + 1] - gridy[ey]) / 2 * (self.yquad + 1)
                )
                jacobian_x = (gridx[ex + 1] - gridx[ex]) / 2
                jacobian_y = (gridy[ey + 1] - gridy[ey]) / 2
                jacobian = ((gridx[ex + 1] - gridx[ex]) / 2) * (
                    (gridy[ey + 1] - gridy[ey]) / 2
                )

                u_NN_quad_element = self.net_u(x_quad_element, y_quad_element)
                d1xu_NN_quad_element, d2xu_NN_quad_element = self.net_dxu(
                    x_quad_element, y_quad_element
                )
                d1yu_NN_quad_element, d2yu_NN_quad_element = self.net_dyu(
                    x_quad_element, y_quad_element
                )
                ...
                d1testy_quad_element, d2testy_quad_element = self.dTest_fcn(
                    Ntest_elementy, self.yquad
                )

                integrand_1 = (
                    d2xu_NN_quad_element + d2yu_NN_quad_element
                )  # Diffusion term
                integrand_2 = (V * d1xu_NN_quad_element) + (
                    W * d1yu_NN_quad_element
                )  # Advection term

                if var_form == 0:
                    U_NN_element_Dif = tf.convert_to_tensor(
                        [
                            [
                                jacobian
                                * tf.reduce_sum(
                    ...
                    U_NN_element_Adv = tf.convert_to_tensor(
                        [
                            [
                                jacobian
                                * tf.reduce_sum(
                    ...
                    U_NN_element = U_NN_element_Adv - eps * U_NN_element_Dif
                        ...
                        [
                            [
                                ...
                                * tf.reduce_sum(
                                    ...
                                    * testx_quad_element[r]
                                    * self.wquad[:, 1:2]
                                    * d1testy_quad_element[k]
                                    * d1yu_NN_quad_element
                                )
                                for r in range(Ntest_elementx)
                            ]
                            for k in range(Ntest_elementy)
    ...
    def train(self, nIter):
        ...
        # textOut = open(textOutFname, 'w')

        # textOut.write('It, Loss, Time')
        ...
        for it in range(nIter):
            ...
            if it % 100 == 0:
                ...
                str_print = "".join(["%d, %.3e, %.2f"])
                # textOut.write(str_print % (it, loss_value, elapsed))
                print(str_print % (it, loss_value, elapsed))
                start_time = time.time()

        # textOut.close()
        # with open('model_WB.pkl', 'wb') as file:
        # pickle.dump([w.numpy().tolist() for w in self.weights], file)
        # tf.compat.v1.train.Saver(self.sess, 'my_model')
        # mlflow.log_artifact(f"{textOutFname}")

        # print(self.layers)

    def predict(self):
        u_pred = self.sess.run(
            self.u_test, {self.x_test: self.xtest, self.y_test: self.ytest}
        )
        return u_pred
...
if __name__ == "__main__":
    ...
    mlflowExptName = input()

    # # Input e power
    # e_power = 1  # int(input())
    # N_train = 200  # int(input())*1000
    # eps = 10**(-e_power)
    # textOutFname = 'out_log.csv'

    # # Input e power
    # e_power = 1  # int(input())
    N_train = 150  # int(input())*1000
    eps = 10 ** (-4)
    # textOutFname = 'out_log.csv'
    ...
    mlflow.set_experiment(f"{mlflowExptName}")
    mlflow.log_param("Epsilon", eps)
    mlflow.log_param("Max Iter", N_train)
    # mlflow.log_artifact(textOutFname)
    ...
    def f_ext(x, y):
        ...
        # gtemp = np.ones(x.shape)
        gtemp = -0.5 * np.pi * (
            V * np.sin(0.5 * np.pi * x) * np.cos(0.5 * np.pi * y)
            + W * np.sin(0.5 * np.pi * y) * np.cos(0.5 * np.pi * x)
        ) + 2 * eps * (0.5 * np.pi) ** 2 * (
    ...
    for ex in range(NE_x):
        for ey in range(NE_y):
            ...
            x_quad_element = grid_x[ex] + (grid_x[ex + 1] - grid_x[ex]) / 2 * (
                x_quad + 1
            )
            y_quad_element = grid_y[ey] + (grid_y[ey + 1] - grid_y[ey]) / 2 * (
                y_quad + 1
            )
            jacobian = ((grid_x[ex + 1] - grid_x[ex]) / 2) * (
                (grid_y[ey + 1] - grid_y[ey]) / 2
            )

            testx_quad_element = np.asarray(
                [Test_fcn_x(n, x_quad) for n in range(1, Ntest_elementx + 1)]
            )
            testy_quad_element = np.asarray(
                [Test_fcn_y(n, y_quad) for n in range(1, Ntest_elementy + 1)]
            )

            u_quad_element = u_ext(x_quad_element, y_quad_element)
            f_quad_element = f_ext(x_quad_element, y_quad_element)

            U_ext_element = np.asarray(
                [
                    [
                        jacobian
                        * np.sum(
    ...
    data_temp = np.asarray(
        [
            [[xtest[i], ytest[j], u_ext(xtest[i], ytest[j])] for i in range(len(xtest))]
    ...
    fig = plt.figure(1)
    plt.title(
        "Loss vs iteration for eps =" + str(eps) + ",V = " + str(V) + ",W = " + str(W)
    )
    ...
    plt.savefig("".join(["CD2D_", scheme, "_loss", ".png"]))
    mlflow.log_artifact("".join(["CD2D_", scheme, "_loss", ".png"]))
    ...
    # fig.tight_layout()
    fig.set_size_inches(w=11, h=11)
    plt.savefig("".join(["CD2D_", scheme, "_Domain", ".png"]))
    mlflow.log_artifact("".join(["CD2D_", scheme, "_Domain", ".png"]))
    ...
    CS_ext = ax_ext.contourf(
        x_test_plot, y_test_plot, u_test_plot, 100, cmap="jet", origin="lower"
    )
    ...
    plt.savefig("".join(["CD2D_", scheme, "_Exact", ".png"]))

    fig_pred, ax_pred = plt.subplots(constrained_layout=True)
    CS_pred = ax_pred.contourf(
        x_test_plot, y_test_plot, u_pred_plot, 100, cmap="jet", origin="lower"
    )
    ...
    plt.title(
        "Predicted u(x,y) values for  eps ="
        + str(eps)
        + ",V = "
        + str(V)
        + ",W = "
    ...
    plt.savefig("".join(["CD2D", scheme, "_Predict_", ".png"]))
    mlflow.log_artifact("".join(["CD2D", scheme, "_Predict_", ".png"]))

    fig_err, ax_err = plt.subplots(constrained_layout=True)
    CS_err = ax_err.contourf(
        x_test_plot,
        y_test_plot,
        abs(u_test_plot - u_pred_plot),
        100,
        cmap="jet",
        origin="lower",
    )
    CS_err = ax_err.contourf(
        ...
        y_test_plot,
        abs(
            np.cos(0.5 * np.pi * x_test_plot) * np.cos(0.5 * np.pi * y_test_plot)
            - u_pred_plot
        ),
        100,
        cmap="jet",
    ...
    plt.savefig("".join(["CD2D_", scheme, "_PntErr", ".png"]))
    mlflow.log_artifact("".join(["CD2D_", scheme, "_PntErr", ".png"]))
