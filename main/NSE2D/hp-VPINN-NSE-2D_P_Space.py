...
class VPINN:
    def __init__(self, coord_bound_train, u_bound_train, v_bound_train, coord_train_force, f_1_train, f_2_train, coord_quad_train, quad_weight_train, F_1_exact_total, F_2_exact_total,\
                 gridx, gridy, num_total_testfunc, num_total_testfunc_p, coord_test, u_test,v_test,p_test, layers):
        ...
        self.num_test_x_p = num_total_testfunc_p[0][0]          # number of test functions in x direction. Data type: int, Size: (1)
        self.num_test_y_p = num_total_testfunc_p[1][0]          # number of test functions in y direction. Data type: int, Size: (1)
        ...
        for ex in range(self.num_element_x): # loop over the elements in x direction
            for ey in range(self.num_element_y): # loop over the elements in y direction
                ...
                Ntest_elementx_p = num_total_testfunc_p[0][ex]  # number of test functions in x direction. Data type: int, Size: (1)
                Ntest_elementy_p = num_total_testfunc_p[1][ey]  # number of test functions in y direction. Data type: int, Size: (1)
                ...
                testx_quad_element_p = self.Test_fcnx(Ntest_elementx_p, self.x_quad)                              # test functions in x direction. Data type: numpy.ndarray, Size: (Ntest_elementx,N_quad)
                d1testx_quad_element_p, d2testx_quad_element_p = self.grad_test_func(Ntest_elementx_p, self.x_quad) # first and second derivatives of the test functions in x direction. Data type: numpy.ndarray, Size: (Ntest_elementx,N_quad)
                testy_quad_element_p = self.Test_fcny(Ntest_elementy_p, self.y_quad)                              # test functions in y direction. Data type: numpy.ndarray, Size: (Ntest_elementy,N_quad)
                d1testy_quad_element_p, d2testy_quad_element_p = self.grad_test_func(Ntest_elementy_p, self.y_quad) # first and second derivatives of the test functions in y direction. Data type: numpy.ndarray, Size: (Ntest_elementy,N_quad)
    
                if var_form == 3:
                    # \int_{\Omega} \epsilon * \grad{c} \cdot \grad{v} dx dy = \int_{\Omega} \epsilon *  d/dx(c)* d/dx(vx) * vy dx dy
                    # \sum
                    U_NN_element_diff_1 = tf.convert_to_tensor([[self.Re*jacobian/jacobian_x*tf.reduce_sum(\
                    ...
                    U_NN_element_diff_2 = tf.convert_to_tensor([[self.Re*jacobian/jacobian_y*tf.reduce_sum(\
                    ...
                    V_NN_element_diff_1 = tf.convert_to_tensor([[self.Re*jacobian/jacobian_x*tf.reduce_sum(\
                                    self.w_quad[:,0:1]*d1testx_quad_element[r]*self.w_quad[:,1:2]*testy_quad_element[k]*d1xv_NN_quad_element) \
                                    for r in range(Ntest_elementx)] for k in range(Ntest_elementy)], dtype= tf.float64) # Data type: tf.tensor, Size: (Ntest_elementx,Ntest_elementy)
                    V_NN_element_diff_2 = tf.convert_to_tensor([[self.Re*jacobian/jacobian_y*tf.reduce_sum(\
                    ...
                    Continuity_NN_element = tf.convert_to_tensor([[jacobian*tf.reduce_sum(\
                                    self.w_quad[:,0:1]*testx_quad_element_p[r]*self.w_quad[:,1:2]*testy_quad_element_p[k]*(d1xu_NN_quad_element+d1yv_NN_quad_element)) \
                                    for r in range(Ntest_elementx_p)] for k in range(Ntest_elementy_p)], dtype= tf.float64) # Data type: tf.tensor, Size: (Ntest_elementx,Ntest_elementy)
...
if __name__ == "__main__":     
    ...
    num_total_testfunc_p = [N_test_x_p, N_test_y_p]
...
#    U_ext_total = np.reshape(U_ext_total, [N_el_x, N_el_y, N_test_y, N_test_x])
    ...
    model = VPINN(coord_all_train, u_all_train,v_all_train, coord_train_force, f_1_train, f_2_train, quad_coord_train, quad_weight_train,\
                 F_1_ext_total, F_2_ext_total, grid_x, grid_y, num_total_testfunc, num_total_testfunc_p, coord_test, u_test,v_test,p_test, Net_layer)
